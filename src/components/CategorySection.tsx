...
export default function CategorySection({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Escape') { setInputValue(''); setError(''); setShowInput(false); }
  };
    ...
    <div className="bg-white rounded-[28px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-slate-50/50 relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-4 relative z-10">
        ...
        <button
          ...
          className={`text-sm flex items-center gap-1 font-bold transition-all px-3 py-1.5 rounded-full ${
            showInput ? 'bg-red-50 text-red-500' : 'bg-slate-50 text-slate-500 hover:text-[#84B741]'
          }`}
        >
          {showInput ? (
            <><X size={14} /> 取消</>
      ...
      <div className="relative z-10">
        {/* 横向滚动列表 */}
        <div className="flex gap-3 overflow-x-auto hide-scrollbar -mx-5 px-5 py-1">
          {items.map((item) => (
        ...
        {/* 输入框区 */}
        <div className={`overflow-hidden transition-all duration-300 ${showInput ? 'max-h-[120px] opacity-100 mt-4 pt-4 border-t border-slate-100' : 'max-h-0 opacity-0'}`}>
